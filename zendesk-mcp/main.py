"""Zendesk MCP — Streamable HTTP at /mcp-server. Cloud Run IAM (verified OIDC) and/or X-API-Key."""
...
ENV = os.getenv("ENV", "")
API_KEY_SECRET_ID = os.getenv("API_KEY_SECRET_ID", "zendesk-mcp-api-key")
...
def _cloud_run_caller_authorized_sync(request: Request) -> bool:
    """True if Authorization Bearer is a valid Google ID token for this host and an allowlisted invoker SA."""
    if ENV == "local":
        return False
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    """Load API key and optionally pre-warm ZendeskService; run MCP session manager."""
    global API_KEY
    ...
    try:
        from zendesk_client import ZendeskService

        await asyncio.to_thread(ZendeskService)
        logger.info("ZendeskService pre-warmed")
    except Exception as e:
        logger.warning("Could not pre-warm ZendeskService (first tool call will initialize): %s", e)

    async with mcp.session_manager.run():
        yield

    logger.info("zendesk-mcp shutdown complete")


app = FastAPI(title="Zendesk MCP", lifespan=lifespan)
...
async def mcp_api_key_guard(request: Request, call_next):
    """Allow MCP if X-API-Key matches or request is a verified Cloud Run invoker (mcp-proxy OIDC)."""
