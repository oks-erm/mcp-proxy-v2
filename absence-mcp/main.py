"""absence.io MCP — Streamable HTTP at /mcp-server. Cloud Run IAM (verified OIDC) and/or X-API-Key."""
...
ENV = os.getenv("ENV", "")
API_KEY_SECRET_ID = os.getenv("API_KEY_SECRET_ID", "absence-mcp-api-key")
ABSENCE_HAWK_ID_SECRET_ID = os.getenv("ABSENCE_HAWK_ID_SECRET_ID", "absence-api-key-id")
ABSENCE_HAWK_KEY_SECRET_ID = os.getenv("ABSENCE_HAWK_KEY_SECRET_ID", "absence-api-key")
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    if ENV == "local":
        ...
        hawk_id = os.getenv("ABSENCE_HAWK_ID") or os.getenv("ABSENCE_API_KEY_ID")
        hawk_key = os.getenv("ABSENCE_HAWK_KEY") or os.getenv("ABSENCE_API_KEY")
        if not hawk_id or not hawk_key:
            logger.warning(
                "ENV=local but ABSENCE_HAWK_ID/ABSENCE_HAWK_KEY (or ABSENCE_API_KEY_ID/ABSENCE_API_KEY) not set; "
                "tools will fail until configured"
            )
        absence_client.configure_api(
            hawk_id,
            hawk_key,
            os.getenv("ABSENCE_API_BASE_URL"),
        )
    else:
        ...
        try:
            hid = get_secret(ABSENCE_HAWK_ID_SECRET_ID).strip()
            hkey = get_secret(ABSENCE_HAWK_KEY_SECRET_ID).strip()
            absence_client.configure_api(hid, hkey, os.getenv("ABSENCE_API_BASE_URL"))
            logger.info("absence.io Hawk credentials loaded")
        except Exception as e:
            logger.warning(
                "Could not load absence.io Hawk secrets (%s, %s): %s. Tools will return errors until configured.",
                ABSENCE_HAWK_ID_SECRET_ID,
                ABSENCE_HAWK_KEY_SECRET_ID,
                e,
            )
            absence_client.configure_api(None, None, os.getenv("ABSENCE_API_BASE_URL"))

    async with mcp.session_manager.run():
        yield

    logger.info("absence-mcp shutdown complete")


app = FastAPI(title="absence.io MCP", lifespan=lifespan)
