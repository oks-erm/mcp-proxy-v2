"""Breezeway MCP — Streamable HTTP at /mcp-server. Cloud Run IAM (verified OIDC) and/or X-API-Key."""
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    if ENV == "local":
        ...
        try:
            ...
            logger.warning(
                ...
                "X-API-Key auth is disabled until the secret exists and breezeway-mcp-sa has "
                "roles/secretmanager.secretAccessor on it. OIDC (mcp-proxy) access still works.",
                API_KEY_SECRET_ID,
                e,
            )
            API_KEY = None
    ...
    try:
        ...
        await asyncio.to_thread(BreezewayMcpClient)
        logger.info("BreezewayMcpClient pre-warmed")
    except Exception as e:
        logger.warning("Could not pre-warm BreezewayMcpClient (first tool call will initialize): %s", e)
